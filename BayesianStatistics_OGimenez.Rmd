...
`r insert_inc_bullet()` The chance of a negative test given you are not a is $\Pr(-|\text{mortal}) = 0.95$ (**specificity**).
...
`r insert_inc_bullet()` From the perspective of the person: Given that the test is positive, what is the probability that this person is a vampire? $\Pr(\text{vampire}|+) = \; ?$
...
- $\Pr(\text{vampire}|+) = \Pr(\text{vampire and } +) / \Pr(+)$
...
## Dynamic updating: _Today's posterior is tomorrow's prior_.
