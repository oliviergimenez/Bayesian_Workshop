...
* Ruth King, Byron Morgan, Steve Brooks (our workshops and [_Bayesian analysis for population ecology_ book](https://www.maths.ed.ac.uk/~rking33/Book-website/index.html)).

* Richard McElreath ([_Statistical rethinking_ book and lecture videos](https://github.com/rmcelreath/statrethinking_winter2019)).

* Jim Albert and Jingchen Hu ([_Probability and Bayesian modelling_ book](https://bayesball.github.io/BOOK/probability-a-measurement-of-uncertainty.html)).

* Materials shared by [Tristan Marh](https://www.tjmahr.com/), [Jason Matthiopoulos](https://www.gla.ac.uk/researchinstitutes/bahcm/staff/jasonmatthiopoulos/), [Francisco Rodriguez Sanchez](https://frodriguezsanchez.net/), [Kerrie Mengersen](https://staff.qut.edu.au/staff/k.mengersen) and [Mark Lai](https://quantscience.rbind.io/).

## Slides codes and data

* All material prepared with `R`.
* `R Markdown` used to write reproducible material.
* Slides available on FigShare [https://doi.org/10.6084/m9.figshare.12656894.v1](https://doi.org/10.6084/m9.figshare.12656894.v1).
* Material available via [https://github.com/oliviergimenez/Bayesian_Workshop](https://github.com/oliviergimenez/Bayesian_Workshop).

## Why are you here?

* We will try and demystify Bayesian statistics, and what we call MCMC.
* You will be able to make the difference between Bayesian and Frequentist analyses.
* You will be able to understand the Methods section of ecological papers doing Bayesian stuff.
* You will be able to run your own Bayesian analysis, safely hopefully.
...
`r insert_inc_bullet()` The chance of a positive test given covid $\Pr(+|\text{covid}) = 0.90$ (**sensitivity**).

`r insert_inc_bullet()` The chance of a negative test given no covid $\Pr(-|\text{no covid}) = 0.95$ (**specificity**).


## What is the question?

`r insert_inc_bullet()` From the perspective of the test: Given a person has covid, what is the probability that the test is positive? $\Pr(+|\text{covid}) = 0.90$.

`r insert_inc_bullet()` From the perspective of the person: Given that the test is positive, what is the probability that a person has covid? $\Pr(\text{covid}|+) = \; ?$

`r insert_inc_bullet()` Assume $0.1\%$ of the population has a chance of getting covid, that is $\Pr(\text{covid}) = 0.001$.
...
- $\Pr(\text{covid}|+) = \Pr(\text{covid and } +) / \Pr(+)$

- $\Pr(\text{covid and } +) = \Pr(\text{covid}) \; \Pr(+ | \text{covid}) = 0.0009$

- $\Pr(+) = 0.0009 + 0.04995 = 0.05085$

- $\Pr(\text{covid}|+) = 0.0009/0.05085 = 0.02$

\endcol
\endcols

$$\Pr(\text{covid}|+)= \displaystyle{\frac{ \Pr(+|\text{covid}) \; \Pr(\text{covid})}{\Pr(+)}}$$

# Your turn

## Screening for covid-19

* Suppose the diagnostic test has the same sensitivity and specificity but the covid is more common: $10\%$ of the population has a chance of getting the covid. 

* What is the probability that a person has covid, given that the test is positive?

# Solution

## The probability that a person has covid, given that the test is positive

- $\Pr(+ | \text{covid}) = 0.9$
- $\Pr(- | \text{no covid}) = 0.95$
- $\Pr(\text{covid}) = 0.1$

$$\begin{aligned}
 \Pr(+) &= \Pr(+ | \text{covid}) \Pr(\text{covid}) + \Pr(+ | \text{no covid}) \Pr(\text{no covid}) \\
  &= 0.9*0.1 + 0.05*0.9 \\
  &=0.135
\end{aligned}
$$
...
$$\begin{aligned}
 \Pr(\text{covid} | +) &= \Pr(+ | \text{covid}) \Pr(\text{covid}) / \Pr(+) \\
  &= 0.9*0.1 / 0.135
\end{aligned}
$$
...
## Reverend Thomas Bayes, back to the 18th century
```{r, out.width = '15cm',out.height='6cm',fig.align='center',echo=FALSE}
...
* There is always a set of parameters that gives you the highest likelihood of observing the data: the \alert{Maximum Likelihood Estimates (MLEs)}
...
* We assume a Normal distribution for the model. 
...
`r insert_inc_bullet()` Thus, a suitable prior distribution might be the Beta which is defined on this range $[0,1]$.
...
`r insert_inc_bullet()` Posterior distribution of survival is $\theta\mid k \sim Beta(a+k,b+n-k)$
...
## Case study: European dippers in Eastern France (1981-1987)
...
`r insert_inc_bullet()` **With 3 first years only**
...
Now use $\phi_{prior} \sim \text{Beta}(\alpha,\beta)$ instead of $\phi_{prior} \sim \text{Normal}(0.57,0.073^2)$
...
## Prior predictive checks: linear regression
...
## Prior predictive checks: logistic regression
...
* Change sampler. Upgrade to Nimble or Stan.
...
`r insert_inc_bullet()` Idea: \alert{penalize models with too many parameters}.
...
`r insert_inc_bullet()` Two models are difficult to distinguish if $\Delta AIC < 2$.
...
`r insert_inc_bullet()` \alert{Multilevel models} include both fixed and random effects

`r insert_inc_bullet()` Random effects are statistical parameters that attempt to \alert{explain noise caused by clusters} of the population you are trying to model

`r insert_inc_bullet()` A multilevel model assumes that the dataset being analysed consists of \alert{a hierarchy of different populations} whose differences relate to that hierarchy

`r insert_inc_bullet()` Measurement that come \alert{in clusters} or groups
...
\alert{Model with complete pooling. All clusters the same.}
...
\alert{Model with no pooling. All clusters unrelated (fixed effect).}

## GL\alert{M}M of GLM with partial pooling
...
\alert{Model with partial pooling. Clusters are somehow related (random effect).}
...
`r insert_inc_bullet()` \alert{Shrinkage via pooling is desirable}. The no-pooling model overstates variation among clusters and makes the
...
`r insert_inc_bullet()` We can \alert{generalize to a wider population}. Is there an allometry relationship between number of seeds and biomass?

`r insert_inc_bullet()` We may consider \alert{varying slopes}. We'd need to deal with correlations between intercept and slope random effects. Open a whole new world with spatial (or time) autocorrelation, phylogenetic regressions, quantitative genetics, network models.

`r insert_inc_bullet()` We may \alert{include predictors at the cluster level}. Imagine we know something about functional traits, and wish to determine whether some species-to-species variation in the allometry relationship is explained by these traits. 
...
* With great tools come great responsabilities
  ...
  + Computational burden can be high (see function `R2jags::jags.parallel()` and package [`jagsUI`](https://github.com/kenkellner/jagsui).
...
## [Why become a bayesian? Ask twitter!](https://twitter.com/ChelseaParlett/status/1282798645453000704)
